...
class OkxWsClient:
    ...
    async def setup_grid_orders(self):
        ...
        try:
            logger.info(f"Cancelando órdenes abiertas para {self.symbol} (excepto Take Profits)...")
            try:
                open_orders = await self.exchange.fetch_open_orders(self.symbol)
                orders_to_cancel = []
                for order in open_orders:
                    # En OKX, reduceOnly puede venir en el objeto de CCXT o en 'info'
                    is_reduce_only = order.get('reduceOnly', False)
                    info = order.get('info', {})
                    if info.get('reduceOnly') == 'true' or info.get('posSide') in ['close_long', 'close_short']:
                        is_reduce_only = True
                ...
                if orders_to_cancel:
                    logger.info(f"Cancelando {len(orders_to_cancel)} órdenes previas que aumentan posición...")
            ...
            leverage = float(self.ai_recommendation.get('leverage', 15.0))
            logger.info(f"Configurando leverage {leverage}x para {self.symbol}...")
            try:
                await self.exchange.set_leverage(leverage, self.symbol)
            except Exception as e:
                logger.warning(f"No se pudo setear leverage: {e}")
            
            grid_lines = int(self.ai_recommendation.get('grid_lines', 10))
            grid_spacing = float(self.ai_recommendation.get('grid_spacing_factor', 0.5)) / 100.0
            
            effective_investment = self.base_capital * leverage
            usd_per_line = effective_investment / grid_lines
            
            # Necesitamos traer los markets para usar amount_to_precision y price_to_precision
            await self.exchange.load_markets()
            
            current_price = self.metrics['last_price']
            spacing = current_price * grid_spacing
            
            market = self.exchange.market(self.symbol)
            contract_size = market.get('contractSize', 1)
            
            # Amount de contratos: (usd / price) / contractSize
            raw_amount = (usd_per_line / current_price) / contract_size
            amount_str = self.exchange.amount_to_precision(self.symbol, raw_amount)
            amount = float(amount_str)
            if amount <= 0:
                logger.warning(f"El capital por línea no es suficiente para 1 contrato en {self.symbol}. Se usará el mínimo de 1 contrato.")
                amount = 1.0 # fallback al minimo de contrato
                
            logger.info(f"Creando batch de {grid_lines} órdenes Limit (Spacing: {spacing:.2f}, Inversión por línea: ${usd_per_line:.2f}, Cantidad: {amount} cont.)...")
            
            orders = []
            # Determinar distribución: 100% NEUTRAL (50/50)
            # Ignoramos la dirección de la IA para operar en ambos sentidos por igual
            buy_lines = grid_lines // 2
            sell_lines = grid_lines - buy_lines
            
            logger.info(f"Dirección forzada a NEUTRAL -> {buy_lines} Buy, {sell_lines} Sell")
            
            # Órdenes de COMPRA abajo del precio
            for i in range(1, buy_lines + 1):
            ...
            # Órdenes de VENTA arriba del precio
            for i in range(1, sell_lines + 1):
                price = current_price + (i * spacing)
                orders.append({
                    'symbol': self.symbol,
            ...
            created_orders = []
            if self.exchange.has['createOrders']:
                logger.info(f"Enviando {len(orders)} órdenes en Batch vía ccxt createOrders...")
                created_orders = await self.exchange.create_orders(orders)
            else:
                logger.info(f"Enviando {len(orders)} órdenes concurrentemente (createOrder)...")
                tasks = []
                for o in orders:
                    tasks.append(self.exchange.create_order(o['symbol'], o['type'], o['side'], o['amount'], o['price']))
                created_orders = await asyncio.gather(*tasks, return_exceptions=True)
                
            logger.info(f"✅ {len(created_orders)} órdenes creadas en el exchange.")
            
            # Iniciar el reloj de inactividad al crear/re-centrar la malla
            self.ultima_ejecucion_ts = time.time()
            self.malla_modificada = True
